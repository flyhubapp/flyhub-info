  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        ...
        const response = await fetch(`${apiUrl}/legal/shipping`);
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
        const data = await response.json();
        
        console.log('📄 Legal API Data (Shipping):', data);
      ...
      <main className="privacy-policy-content">
        {loading ? (
          <div className="loading-container">
            ...
            <p>Fetching latest shipping policy...</p>
          </div>
        ) : content ? (
          <div 
          ...
          <div className="empty-content">
            <p>Shipping Policy content has not been updated yet.</p>
          </div>
        )}
      </main>
    </div>
